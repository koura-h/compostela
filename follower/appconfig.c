...
az_list* g_config_channels = NULL;

sc_config_channel_entry*
_pick_channel_entry(yaml_parser_t* parser)
{
    sc_config_channel_entry* ret =
    (sc_config_channel_entry*)malloc(sizeof(sc_config_channel_entry));

    int done = 0;
    // int error = 0;
    yaml_event_t event, event_value;

    memset(ret, 0, sizeof(sc_config_channel_entry));
    ...
    while (!done) {
        ...
        case YAML_SCALAR_EVENT:
            ...
            if (strcmp((const char*)event.data.scalar.value, "name") == 0) {
                ret->name = strdup((const char*)event_value.data.scalar.value);
            } else if (strcmp((const char*)event.data.scalar.value, "path") == 0) {
...
az_list*
_pick_channels(yaml_parser_t* parser)
{
    az_list* li = NULL;
    int done = 0;
    int error = 0;
    ...
    while (!done) {
        ...
        case YAML_MAPPING_START_EVENT:
            {
                sc_config_channel_entry *entry = _pick_channel_entry(parser);
                if (entry) {
...
_pick_global(yaml_parser_t* parser)
{
    ...
    while (!done) {
        ...
        if (strcmp((const char*)event.data.scalar.value, "server") == 0) {
            ...
            g_config_channels = _pick_channels(parser);
        }
